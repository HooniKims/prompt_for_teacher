...
- 로컬 모델 사용 시 최종 결과는 짧은 완료 신호를 받은 뒤 `교사용 요약`, `AI 실행용 프롬프트`, `참고 안내`를 단계별로 나누어 생성
- OpenAI `gpt-5.4-nano` 기본 흐름은 기존처럼 한 번에 최종 결과를 생성
...
- e4b 샘플 데모 확인
- 로컬 e4b 질문 생성은 768 토큰으로 제한하고, 응답 지연 시 10단계 기본 질문으로 이어가도록 fallback 적용
- 로컬 e4b 최종 프롬프트는 교사용 요약, AI 실행용 프롬프트, 참고 안내를 나누어 생성하되, 10단계 확인 전에는 조기 완료되지 않도록 차단
- 내장 브라우저에서 e4b로 천천히 10단계 입력 테스트 완료: `10 / 10`, 교사용 요약, AI 실행용 프롬프트, 개인정보보호/학습지원 소프트웨어 심의 참고 안내 생성 확인
- 로컬 e4b API 짧은 응답 10회 연속 점검 완료: `10 / 10` 정상 응답, HTTP 오류와 빈 응답 없음
- AI 실행용 프롬프트는 개발에 바로 들어갈 수 있도록 기능별 모듈, 데이터 모델, 화면 흐름, 권한 설계, 개인정보보호 구현 계획, 심의 대응 구현 계획, 테스트 기준을 포함
- 참고 안내에는 교사 단독 행정·업무·수업 준비용 도구는 심의 제외, 정규 수업에서 학생과 함께 쓰며 개인정보를 수집·처리하는 도구는 심의 필요, 개인정보 미수집 단순 도구는 심의 제외 가능이라는 판단 흐름을 포함
- 교사 자체 개발 수업용 도구가 개인정보를 수집·처리하는 경우 교사가 개인정보 보호 5개 필수 기준 체크리스트나 서술형 증빙을 준비해야 한다는 안내 반영
